...
由于距离向分辨率较大，载波频率较高，$N_{el}$ 的上限较为宽松，而其下限由信号带宽与分辨率决定。对 $N_{el}$ 的选择可以通过遍历所有可能的 $N_{el}$ 来选取可用的通道数，从而得到较为合适的值。本例中选取 $N_{el}$ 为 10，对应的 $\tau = -2.629$ ns。所使用的信号带宽 $B = 400 $ MHz 。为了使NESZ达到要求，发射总功率为 10000 W，即每个天线单元为 1000W 。其中各性能参数为
