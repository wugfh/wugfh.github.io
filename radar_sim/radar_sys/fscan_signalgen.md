...
$$|k| \approx \frac{4\rho_r(\phi)f_c}{Nc}$$

因此可以得到 $k$ 的大致取值范围

$$0 < |k| < \frac{4\rho_{max}f_c}{Nc} , k \in \mathbb{Z}$$
...
天线单元相位间距 $d = 0.098m$ ， 天线单元时延 $\tau = -1.46ns$
</center>

可以发现，对于成像区域的近端，由于带宽的限制，只有单独一个栅瓣进行扫描。而对于远端，便存在两个栅瓣在不同的频段进行扫描。设近端扫描栅瓣序号为 $k_1$, 远端栅瓣扫描情况为 $k_2$ , 满足 $k_1 = k_2+1$ 。则 $k_1$ 栅瓣由于受到最大可能频率 $f_c+B/2$ 的限制，最远只能扫描到 $\phi_{1,max}$ 。同理 $k_2$ 栅瓣由于受到最小可能频率 $f-B/2$ 的限制，最近只能从 $\phi_{2,min}$ 开始扫描，导致整个成像区域由多个栅瓣共同扫描完成。为使得讨论比较方便，这里假设 $k_1,\enspace k_2$ 均大于0，$k_1,\enspace k_2$ 的符号不影响讨论的最后结果。由之前所诉，$h(f,\phi)$ 的两个栅瓣的扫描区域满足以下条件
...
在天线子孔径满足上述条件后，便存在时延 $\tau$ 使得该模糊情况不在存在。
...
由于距离向分辨率较大，载波频率较高，$N_{el}$ 的上限较为宽松，而其下限由信号带宽与分辨率决定。
